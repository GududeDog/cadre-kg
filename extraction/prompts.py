...
- unit: 工作单位全称
- department: 所在科室/部门
- position: 担任职务名称（只提取职务名称原文，如"副乡长""科长"，不推断级别）
- region: 所在地区
- dept: 科室简称
- rank: 统一填空字符串""（职务级别和职级级别将由后续规则引擎推断，本次不需要提取）

## 注意事项
1. 简历通常以表格形式呈现，每一行是一段任职经历
2. 时间统一为 YYYY.MM 格式，"2003年7月"→"2003.07"，"74.08"→"1974.08"
3. 职务名称只从 position 字段提取原文，rank 字段一律填空，不要尝试推断职级
...
POSITION_RANK_PROMPT = """你是一个干部职务职级推断专家。
给定一段"现任职务"文本，根据以下规则推断职务级别和职级级别。
...
## 重要提醒
1. 职务和职级是两个独立的推断，互不干扰
2. 职务从"单位+岗位"综合推断，职级从职级关键词推断
3. 职级关键词覆盖综合管理类、综合执法类、纪检监察类、检察官、法官助理/书记员、员额法官、执法勤务类警员等全部类别
4. 职级关键词可能在文本的任意位置（逗号、顿号、空格分隔），不限于最后一部分
5. 如果同时出现多个职级关键词，取级别最高的那个
6. 如果某字段无法确定，填空字符串""，不编造
7. position_level 和 rank_level 的字面值必须来自规则中列出的值域

严格按照 JSON 格式输出，不要包含其他文字。"""

def build_position_rank_prompt(position_text: str,
                               position_rules: list = None,
                               rank_rules: list = None) -> str:
    def _fmt(lst):
        return "\n".join(f"{i+1}. {r}" for i, r in enumerate(lst)) if lst else "从文本中推断"
    prompt = POSITION_RANK_PROMPT
    prompt = prompt.replace("{position_rules}", _fmt(position_rules or []))
    prompt = prompt.replace("{rank_rules}", _fmt(rank_rules or []))
    return prompt + f"""
